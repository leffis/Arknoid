 ...
 */
public class FasterPaddle extends Eatable implements TimerObserver {

    int timeRemain;
    boolean start;
    public FasterPaddle(Integer _itemID, ItemType _type, Model _model, Double _orix, Double _oriy, byte _weight) {
    ...
    public void draw(Canvas canvas) {
        ...
        Drawable d;
        d = model.getResources().getDrawable(R.drawable.speedup_paddle);
    ...
    public void timerNotify(Paddle pad1, Paddle pad2, List<DynamicItem> ditems, List<Item> sitems) {
        if (timeRemain == 350 && start){
            if (this.centerYr <= 1.0 / 2.0) {
                Pair<Double, Double> fasterSpeed = new Pair<Double, Double>(2.0, 0.0);
                ((BasicPaddle)(model.paddle2)).changeSpeedEatable(fasterSpeed);
            }
            if (this.centerYr > 1.0 / 2.0) {
                Pair<Double, Double> fasterSpeed = new Pair<Double, Double>(2.0, 0.0);
                ((BasicPaddle)(model.paddle1)).changeSpeedEatable(fasterSpeed);
            }
            timeRemain--;
        }
        else if(timeRemain == 0 && start){
            if (this.centerYr <= 1.0 / 2.0) {
                Pair<Double, Double> fasterSpeed = new Pair<Double, Double>(0.5, 0.0);
                ((BasicPaddle)(model.paddle2)).changeSpeedEatable(fasterSpeed);
            }
            if (this.centerYr > 1.0 / 2.0) {
                Pair<Double, Double> fasterSpeed = new Pair<Double, Double>(0.5, 0.0);
